...
describe('Escrow', function () {
  it('Should be able to initialize a trade on an owned NFT', async function () {
    ...
    await expect(escrowContract.connect(user1).initializeTrade(nftContract.address, user2.address, 1, '1700000000000000000')).to.not.be.reverted;
  ...
  it('Should NOT be able to initialize a trade someone elses NFT', async function () {
    await nftContract.connect(user1).setApprovalForAll(escrowContract.address, true);
    await expect(escrowContract.connect(user2).initializeTrade(nftContract.address, user2.address, 1, '1700000000000000000')).to.be.reverted;
  });

  it('Should be able to cancel a trade on an owned NFT', async function () {
    await nftContract.connect(user1).setApprovalForAll(escrowContract.address, true);
    await escrowContract.connect(user1).initializeTrade(nftContract.address, user2.address, 1, '1700000000000000000');
  ...
  it('Should NOT be able to change buyer on a non owned NFT', async function () {
    await nftContract.connect(user1).setApprovalForAll(escrowContract.address, true);
    escrowContract.connect(user1).initializeTrade(nftContract.address, user2.address, 1, '1700000000000000000');
  ...
  it('Should NOT be able to change buyer on a deal that is canceled', async function () {
    await nftContract.connect(user1).setApprovalForAll(escrowContract.address, true);
    escrowContract.connect(user1).initializeTrade(nftContract.address, user2.address, 1, '1700000000000000000');
    ...
    await expect(escrowContract.connect(user1).changeBuyer(nftContract.address, user3.address, 1)).to.be.reverted; 
  });

  it('Should NOT be able to change buyer on a deal that is completed', async function () {
    await nftContract.connect(user1).setApprovalForAll(escrowContract.address, true);
    escrowContract.connect(user1).initializeTrade(nftContract.address, user2.address, 1, '1700000000000000000');

    await expect(escrowContract.connect(user2).finalizeTrade(nftContract.address, 1, {
      value: '1700000000000000000',
    })).to.not.be.reverted; 
    expect(await escrowContract.getDealStatusById(nftContract.address, 1)).to.equal(2);

    await expect(escrowContract.connect(user1).changeBuyer(nftContract.address, user3.address, 1)).to.be.reverted; 
  });

  it('Should NOT be able to cancel on a deal that is completed', async function () {
    await nftContract.connect(user1).setApprovalForAll(escrowContract.address, true);
    escrowContract.connect(user1).initializeTrade(nftContract.address, user2.address, 1, '1700000000000000000');
  ...
  it('Should be able to complete the deal as the new buyer', async function () {
    ...
    await expect(escrowContract.connect(user3).finalizeTrade(nftContract.address, 1, {
      value: '1700000000000000000',
    })).to.be.reverted; 
  });
});
